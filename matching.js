// matching.js
// ============================================================================
// 产业科创成果智能匹配引擎 —— 「知识图谱 + NLP」供需自动匹配
// ----------------------------------------------------------------------------
// 实现说明（纯前端、零依赖，可直接静态托管运行）：
//   1. 知识图谱(KG)：节点=需求/成果/领域/知识产权，边=供需-领域-成果-IP 关联；
//   2. NLP 语义匹配：词典分词 + TF-IDF 向量化 + 余弦相似度；
//   3. 多维量化评分：领域匹配 + 语义相似 + 关键词重合 + 成熟度适配 + 知识产权强度；
//   分数与"AI 分析"理由均由规则实时计算，绝非写死。
//   生产环境可将 tokenize() 替换为大模型 embedding，接口已预留。
// ============================================================================
(function (global) {
  'use strict';
  ...
  // 词典最长前缀匹配 + CJK 二元组兜底，兼顾"新入库"文本也能被分词
